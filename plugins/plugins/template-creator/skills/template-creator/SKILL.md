...
- Manage only personal skills under `${NEXTS_HOME:-~/.nexts}/skills`.
...
7. Read the JSON result. Verify that the generated directory contains `SKILL.md`, `artifact-template.json`, `agents/nextsai.yaml`, the retained `assets/reference.<ext>`, and `assets/preview.png`.

## Update workflow

1. Resolve the exact passed artifact-template skill and read its `SKILL.md`, `artifact-template.json`, `agents/nextsai.yaml`, retained reference, and preview. Stop if it is not a direct child of the personal skills directory or if more than one target was passed.
...
- Location: [~/.nexts/skills/artifact-template-artifacts-meeting-notes](/absolute/path/to/.nexts/skills/artifact-template-artifacts-meeting-notes)
- Artifact type: Document

Formatting rules:

- Emit the returned `skillName` as an unquoted `$skill-name` mention so Nexts renders the skill link UI with its display name. Do not wrap the mention in backticks or make it a Markdown link.
