...
async function createTemplateSkill(
  ...
  const skillsRoot = path.join(nextsHome, "skills");
  const releaseLock = await acquireWriteLock(
    path.join(nextsHome, WRITE_LOCK_NAME),
  );
  try {
    const identity =
      request.mode === "update"
...
function getDefaultNextsHome() {
  const homeDir = path.resolve(
    process.env.HOME ?? process.env.USERPROFILE ?? os.homedir(),
  );
  return path.resolve(process.env.NEXTS_HOME ?? path.join(homeDir, ".nexts"));
}
...
async function stageTemplateSkill({
  ...
  try {
    ...
    await Promise.all([
      ...
      fs.writeFile(
        path.join(stagedSkill, "agents", "nextsai.yaml"),
