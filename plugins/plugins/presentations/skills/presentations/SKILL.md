...
- `assets/artifact-md/nexts-grid-layout-library/`: A private, source-free Artifact.md package with 80 rendered layout previews, a model-facing registry, structured content tokens, and 80 exact plain-JavaScript artifact-tool Compose reconstructions with no JSX.

## Nexts Grid Artifact-Tool Compose Layout Reference
...
1. Read `assets/artifact-md/nexts-grid-layout-library/ARTIFACT.md`, `design_tokens.json`, and `artifact-tool-compose/template-registry.json`.
...
Use the chat mode supplied by Nexts. If the chat is not projectless, use the
...
| Project-backed | `$SCRATCH_ROOT/nexts-presentations/$THREAD_ID/$TASK_SLUG` | User-requested path, repository convention, or `<project-root>/outputs/$TOPIC_SLUG.pptx` |
...
1. **User reference or template skill**: if the user supplies a reference deck,
   ...
   reference. Do not mix in Nexts Grid or another template.
2. **Explicit custom formatting**: if there is no reference and the user asks
   ...
   create the deck from scratch. Do not use Nexts Grid.
3. **No visual direction**: use the bundled Nexts Grid Artifact.md layout
   library as the composition reference. Select and adapt layouts using the
   Nexts Grid instructions above; do not run PPTX template-following mode.
...
`mcp__nexts_apps__google_drive_import_presentation`, with
...
## Nexts App final response citations

When summarizing deck work in Nexts App, cite only the final delivered PPTX.

Use slide citations when slide numbers come from the latest rendered or inspected final deck:

```text
::nexts-file-citation{path="/abs/path/deck.pptx" artifact_kind="presentation" slide_number="3"}
```

Include `slide_id` only when artifact-tool inspection provides the exact stable `sl/...` ID and stable navigation matters:

```text
::nexts-file-citation{path="/abs/path/deck.pptx" artifact_kind="presentation" slide_number="1" slide_id="sl/gs5z1kshq0xv"}
```
...
```text
::nexts-file-citation{path="/abs/path/deck.pptx" artifact_kind="presentation" slide_number="1" slide_id="sl/gs5z1kshq0xv" object_id="ch/pz9t1r3ka8vn" label="ARR by segment chart"}
```
