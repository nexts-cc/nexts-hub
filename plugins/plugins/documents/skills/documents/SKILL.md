...
- Use Nexts workspace dependencies for docx artifact work: resolve them through the workspace dependency loader or runtime skill, then treat the returned Node/Python runtimes and package directory as authoritative. Do not use system `node`, system `python`, global npm packages, or repo-local installs.
...
For a net-new Google Docs request, create and visually verify a local `.docx` with this skill first. The native Google Docs deliverable must then be produced by the Google Drive plugin's document import action, `mcp__nexts_apps__google_drive_import_document`, with `upload_mode: "native_google_docs"`.
...
When summarizing document work in Nexts App, cite only the final delivered DOCX.

Use page citations when page numbers come from the latest rendered or inspected final document:

```text
::nexts-file-citation{path="/abs/path/file.docx" artifact_kind="document" page_number="4"}
```
