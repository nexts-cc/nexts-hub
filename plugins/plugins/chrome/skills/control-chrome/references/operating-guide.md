...
## Sessions persist across conversations — check globally before creating one

Chrome Control sessions and tab groups are recorded in the extension's own storage, not scoped to this conversation — a session started in a completely different, earlier conversation is still there now, exactly like one from an earlier turn in this one.

- **At the start of any browser work, first call `session.list` with `{}`.** This returns every session the extension currently knows about, regardless of which conversation created it. Look through the returned sessions' tabs for one already on the target URL or site.
- **If a matching tab/session is found, reuse it** — activate it and act on it directly. Do **not** start a new session just because you don't remember creating one earlier in this specific conversation.
- Call `session.start` **only** when `session.list` returns nothing usable for this task. Every unnecessary `session.start` creates a new, permanent Chrome tab group the user has to manually close later — this is the main source of clutter, avoid it.
- For additional tabs in the same task, use `session.createTab` on the reused (or newly started) session — they join that group. Use `session.get` to see the tabs already in a session.

## Reuse tabs — don't reopen the same site

- Before opening a URL, check the tabs already listed across `session.list` results. **If any tab in any session is already on that URL (or that site), reuse it** — activate it and act on it. Do **not** open a second tab, and do not start a second session, for a page you already have access to.
- Do not navigate to a new URL to reach a page you already have open.

## Cleanup

- Leaving a session/group open when a task ends is fine and expected — it is what lets a later conversation reuse it instead of creating a new one. Only call `session.stop` (with `closeTabs: true`) when the user explicitly wants those tabs closed, or when tabs are clearly disposable intermediates (a search, a login page, an error page) that serve no further purpose to the user.
- Leave open any tab that is a user-facing deliverable (a document/sheet/result the user wanted, a page they asked to keep).
