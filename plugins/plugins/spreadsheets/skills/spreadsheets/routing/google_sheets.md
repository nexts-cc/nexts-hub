...
For a net-new Google Sheets request, create and verify a local `.xlsx` with this skill first. The native Google Sheets deliverable must then be produced by the Google Drive plugin's spreadsheet import action, `mcp__nexts_apps__google_drive_import_spreadsheet`, with `upload_mode: "native_google_sheets"`.
